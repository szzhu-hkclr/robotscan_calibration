...
int main(int argc, char** argv) {
    ...
    try {
        ...
        // Loop over each group from the configuration.
        for (const auto& group : config.group_lists) {
            ...
            // Set the image size if not already set.
            if (image_size.width == 0 && image_size.height == 0) {
                image_size = images[0].size();
            }

            // Note: We disable showing corners unless required.
            auto cb_result = find_chessboard_corners(images, pattern_size, config.square_size, false);
            if (cb_result.image_points.empty()) continue;

            // Read the entire joint angles file for this group.
            ...
            // data-mapping debug code
            // int cnt = 0;
            // for (auto each : joints_records)
            // {
            //     cnt++;
            //     std::cout << "joints[" << cnt << "]:" << std::endl;
            //     for (auto js : each)
            //         std::cout << "  " << js << ",";
            //     std::cout << std::endl;
            // }
        ...
        // Compose the final 4x4 transformation
