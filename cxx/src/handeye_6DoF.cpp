...
int main(int argc, char** argv) {
    ...
    for (const auto& group : config.group_lists) {
        auto images = read_images(config.image_folder + "/" + group);
        auto cb_result = find_chessboard_corners(images, pattern_size, config.square_size);

        if (cb_result.image_points.empty()) continue;

        auto joint_angles = read_joint_angles(config.pose_folder + "/" + group + ".txt");

        for (int idx : cb_result.valid_indices) {
            if (idx >= joint_angles.size()) continue;

            cv::Mat T = robot.forward(joint_angles[idx]);
        ...
        if (image_size == cv::Size(0, 0) && !images.empty())
            image_size = images[0].size();                            
    }

    auto calib_result = calibrate_camera(all_object_points, all_image_points, image_size);
    ...
    cv::calibrateHandEye(REnd2Base, TEnd2Base, RTarget2Cam, TTarget2Cam, 
                        R_cam2gripper, t_cam2gripper, cv::CALIB_HAND_EYE_DANIILIDIS);
